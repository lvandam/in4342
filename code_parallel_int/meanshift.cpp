...
void MeanShift::Init_target_frame(const cv::Mat &frame,const cv::Rect &rect)
{
    ...
    // Matrix kernel;
    init_matrix(kernel, rect.height, rect.width);
    ...
    for(int i = 0; i < rows; i++)
    {
        ...
        for(int j = 0; j < cols; j+=16)
        {
            ...
            for(int g = 0; g < size; g++)
            {
              ...
              {
                // weight.at<uint>(i,j+g) = static_cast<uint>((sqrt(target_model[k][bin_array[g]]/target_candidate.at<uint>(0, bin_array[g]))));
...
cv::Rect MeanShift::track(const cv::Mat &next_frame)
{
    ...
    for(int iter=0;iter<cfg.MaxIter;iter++)
    {
        ...
        //Matrix weight;// = std::inner_product(weight0.begin(), weight0.end(), weight1.begin(), 0);
        //init_matrix(weight, weight0.size(), weight0[0].size());
        // multTimer.Start();
        // TODO: Speed this shit up!
        for (int ii = 0; ii < weight.size(); ++ii)
          for (int jj = 0; jj < weight[0].size(); ++jj)
        ...
        float sum_wij = 0.0;
        float delta_y = 0.0;
        float centre = static_cast<float>((weight.size()-1)/2.0);
        double mult = 0.0;
        ...
        for(int i = 0; i < weight.size(); i++)
        {
            for(int j = 0; j < weight[0].size(); j++)
            {
                ...
                delta_x += static_cast<float>(norm_j*weight[i][j]*mult);
                delta_y += static_cast<float>(norm_i*weight[i][j]*mult);
