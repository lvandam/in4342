...
cv::Rect MeanShift::track(const cv::Mat &next_frame)
{
    ...
    for(int iter = 0; iter < cfg.MaxIter; iter++)
    {
        ...
        timePoolColor.Start();
        #endif
        poolColor(GREEN,(Uint8*) next_frame.ptr(0,0), target_Region.x, target_Region.y, target_Region.width, target_Region.height);
        #ifdef DET_TIMING
        timePoolColor.Stop();
        tPoolColor+= timePoolColor.GetTime();
        #endif
        ...
        timePoolColor.Start();
        #endif
        poolColor(RED,(Uint8*) next_frame.ptr(0,0), target_Region.x, target_Region.y, target_Region.width, target_Region.height);
        #ifdef DET_TIMING
        timePoolColor.Stop();
        tPoolColor+= timePoolColor.GetTime();
        #endif
        ...
        MatrixFloat weight = MatrixFloat(target_Region.height, RowFloat(target_Region.width));
        ...
        for(size_t i = 0; i < (Uint32) target_Region.height; i++)
        {
          for(size_t j = 0; j < (Uint32) target_Region.width; j++)
          {
            weight[i][j] = weight0[i * target_Region.width + j];
          }
        }
        #ifdef DET_TIMING
        timeMerging.Stop();
