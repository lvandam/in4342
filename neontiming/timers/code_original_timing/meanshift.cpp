...
{
    Timer Init_target_frameTimer("Init_target_frame timer");

    Init_target_frameTimer.Start();

    target_Region = rect;
    target_model = pdf_representation(frame,target_Region);

    Init_target_frameTimer.Stop();
    Init_target_frameTimer.Print();
}

float  MeanShift::Epanechnikov_kernel(cv::Mat &kernel)
{
    Timer Epanechnikov_kernelTimer("Epanechnikov_kernel timer");
    Epanechnikov_kernelTimer.Start();
    ...
    Epanechnikov_kernelTimer.Stop();
    Epanechnikov_kernelTimer.Print();
    return kernel_sum;

}
cv::Mat MeanShift::pdf_representation(const cv::Mat &frame, const cv::Rect &rect)
{
    Timer pdf_representationTimer("pdf_representation timer");
    pdf_representationTimer.Start();
    ...
    pdf_representationTimer.Stop();
    pdf_representationTimer.Print();
    return pdf_model;
...
{
    Timer CalWeightTimer("CalWeight timer");
