...
Uint32 * pool_notify_ValBuf = NULL ;
...
{
    Uint16 i;
    for(i=0;i<pool_notify_BufferSize/4;i++) pool_notify_ValBuf[i] = FFracSqrt((pool_notify_ValBuf[i]<<16));
}
...
void floatRootMatrix(void)
{
    ...
    {
        temp = sqrt(pool_notify_ValBuf[i]);
        pool_notify_ValBuf[i]=(Uint32) temp;
    }
}
...
{
    Uint16 i;
    for(i=0;i<pool_notify_BufferSize/4;i++) pool_notify_ValBuf[i] = (i%256) * (i%256);
}
...
NORMAL_API DSP_STATUS pool_notify_Execute (IN Uint32 numIterations, Uint8 processorId)
{
    ...
    Uint32 i;
    Timer initialTimer, fixRootTimer, dspFixRootTimer, floatTimer;   
    ...
    initTimer(&fixRootTimer, "Time elapsed during the computation of fixed point roots on GPP");
    initTimer(&dspFixRootTimer, "Time elapsed during data transfering and computation of fixed point roots on DSP");
    initTimer(&floatTimer, "Time elapsed during the computation of floating point roots on the GPP");

    pool_notify_ValBuf = (Uint32 *) pool_notify_DataBuf;
    startTimer(&initialTimer);
    unit_init();
    stopTimer(&initialTimer);

    for(i=0; i<pool_notify_BufferSize/4; i++) printf("%u\t", pool_notify_ValBuf[i]);
    printf("\n\n");
    
    startTimer(&fixRootTimer);
    fixedRootMatrix();
    stopTimer(&fixRootTimer);
    
    for(i=0; i<pool_notify_BufferSize/4; i++) printf("%f\t", pool_notify_ValBuf[i]/65535.0);
    printf("\n\n");
    
    unit_init();
    
    startTimer(&floatTimer);
    floatRootMatrix();
    stopTimer(&floatTimer);
    
    for(i=0; i<pool_notify_BufferSize/4; i++) printf("%d\t", pool_notify_ValBuf[i]);
    printf("\n\n");
    ...
    printTimer(&fixRootTimer);
    printTimer(&floatTimer);
